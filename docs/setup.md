...
> 1. [Generate Test Metadata](https://github.com/uc-cdis/compose-services/docs/using_the_commons.md#generating-test-metadata)
> 2. Upload the simulated test metadata to the Data Portal UI. Follow [gen3.org](https://gen3.org/resources/user/submit-data/) and [Useful links](https://github.com/uc-cdis/compose-services/docs/useful_links.md) for how-to guides and tutorials. 

> 🟢 Finally, re-enable kibana and guppy services before continuing with the section [Configuring guppy for exploration page](https://github.com/uc-cdis/compose-services/docs/using_the_commons.md#configuring-guppy-for-exploration-page). 
...
```
When you see that `bundle.js` and `index.html` were successfully built in the logs, you should be able to log into https://localhost and see the data commons. You are now ready to setup the [first program and project](https://github.com/uc-cdis/compose-services/docs/using_the_commons.md#programs-and-projects).


## Update tips

You should of course `git pull` compose-services if you have not done so for a while. You also need to `docker-compose pull` new images from Quay--this will not happen automatically. If your git pull pulled new commits, and you already have a `Secrets` folder, you may also need to delete your old `Secrets` and rerun `creds_setup.sh` (see [Setting up Credentials](https://github.com/uc-cdis/compose-services/docs/setup.md#Setting-up-Credentials)) to recreate it.
