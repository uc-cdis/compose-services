...
To run Docker Compose on a remote machine, modify the `BASE_URL` field in `fence-config.yaml`, and the `hostname` field in the `peregrine_creds.json` and `sheepdog_creds.json` in the `Secrets` directory.
